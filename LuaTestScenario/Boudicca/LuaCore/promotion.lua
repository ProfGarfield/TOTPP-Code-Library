...
local function customVetChance(loser,winner,aggressor,victim,loserTile,victimVetStatus,aggressorVetStatus,promotionChanceFn)
    ...
    -- or the winner is not a veteran now, then no need to do anything else
    if (not promotionChanceFn) or winnerVetStatus or (not winner.veteran) then
        return
    end
    ...
    if promotionChance <= gamePromotionChance then
...
local function makeBasicDemotionFunction(demotionTable)
    ...
    for unitTypeID,demotionInfo in pairs(demotionTable) do
        ...
        if demotionTable.destroyIfKilledBy then
            for __,winnerUnitType in pairs (demotionTable.destroyIfKilledBy) do
                newDemotionTable[unitTypeID].replaceIfKilledBy[winnerUnitType.id] = false
            end
        end
        if demotionTable.demoteOnlyIfKilledBy then
            for __,winnerUnitType in pairs(demotionTable.demoteOnlyIfKilledBy) do
                newDemotionTable[unitTypeID].replaceIfKilledBy[winnerUnitType.id] = true
            end
        end
        if not demotionTable.replacementUnitType then
        ...
        newDemotionTable[unitTypeID].replacementUnitType = demotionTable.replacementUnitType
        newDemotionTable[unitTypeID].vetStatus = demotionTable.vetStatus
        newDemotionTable[unitTypeID].damage = demotionTable.damage or 0
        newDemotionTable[unitTypeID].demoteOnlyIfVeteran = demotionTable.demoteOnlyIfVeteran
    end
    local function demotionFunction(loser,winner,loserTile,loserVetStatus,winnerVetStatus)
        ...
        local nDTVal = newDemotionTable[unitTypeID]
...
promotion.checkForUpgrade = promotion.checkForUpgrade
...
local function checkUpgradeInfo(upgradeInfo,tableKey,nilDefeatedUnitTypes,defeatedUnitTypesTable)
    ...
    if upgradeInfo.healUnit and type(upgradeInfo.healUnit) ~="true" and (type(upgradeInfo.healUnit)~="number" or
...
local function buildBasicUpgrade(upgradeInfoTable)
    ...
    local function buildUpgradeDataForWinner(winnerTypeID,upgradeInfo,nilUpgradeInfo)
        ...
        if upgradeInfo.defeatedUnitTypes == nil then
            ...
            for __,unitType in pairs(upgradeInfo) do
        ...
        elseif nilUpgradeInfo then
            ...
            for i=0,civ.cosmic.numberOfUnitTypes do
                if not gen.inTable(civ.getUnitType(i),nilUpgradeInfo.excludedUnitTypes) then
    ...
    local function upgradeFunction(unitToUpgradeID,replacementTypeID,promotionInfoTable)
        ...
        upgradedUnit.moveSpent = math.min(255,upgradeUnit.moveSpent+spendMove)
