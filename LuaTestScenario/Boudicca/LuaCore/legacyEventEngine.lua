...
local function supplyLegacyEventsTable(table)
    if type(table)~="table" then
        ...
        eventTable = table
        --eventHashValue = hash.hash224(civlua.serialize(eventTable))
    end
end
...
--    print(eventHashValue)
--    print(g_LegacyState.storedEventHashValue)
...
--        clearStateDialog:addText("It appears that the legacy events have been changed.  Do you wish to clear the state table for the legacy events?  Doing this will erase all record of previous events (such as continuous flags activated and Just Once events), but will prevent event bugs if you've changed the order of events, or removed events and added new ones.")
