...
--
--  the table reactInfo is where you input the reaction data
--
--
...
details = { {triggerUnitTypes = carrierList, forbiddenUnitTypesOnTileReactionUnit=carrierList,
            --forbiddenUnitTypesOnTileTriggerUnit = {unitAliases.F4F},
            --forbiddenTerrainReactionUnit = allLandTypes,
            --forbiddenTerrainTriggerUnit = allLandTypes,
            reactionRange = 2, hitChance = 1, damageSchedule = gen.makeThresholdTable({[0]=15,}),
            hitChanceTerrainReactionUnit = allLandTypes,
            hitChanceTerrainTriggerUnit = allLandTypes,
            hitChancePriority = "base",
            destroyMunitionsWithKill = true

            }, -- close reactionDetail 1
            {triggerUnitTypes = damagedCarrierList, forbiddenUnitTypesOnTileReactionUnit=carrierList,
            reactionRange = 2, hitChance = 1, damageSchedule = gen.makeThresholdTable({[0]=15,}),
            destroyMunitionsWithKill = true
            },-- close reactionDetail 2
        },--close details key
...
for key,value in pairs(reactInfo) do
    convertReactionInformation(key)
end
--local civlua = require("civlua")
--print(civlua.serialize(reactGovernor))
...
local function canReact(triggerUnit,reactingUnit)
    ...
    if not (reactGovernor[reactingUnit.type.id] and reactGovernor[reactingUnit.type.id][triggerUnit.type.id]) then

        -- reacting unit either can't react to anything, or it can't react to the trigger unit
        return false
    end
        
    print(reactGovernor[reactingUnit.type.id]["reactionsPerTurn"],reactionBase.getReactionsMade(reactingUnit))
    if reactionBase.getReactionsMade(reactingUnit) >= (reactGovernor[reactingUnit.type.id]["reactionsPerTurn"] or math.huge) then
    ...
    if reactionDetail.forbiddenUnitTypesOnTileTriggerUnit then
        ...
        for possibleSpoilerUnit in triggerUnit.location.units do
            if forbiddenTable[possibleSpoilerUnit.type.id] then
                return false
            end
        end
...
local function hitProbability(triggerUnit,reactingUnit)
    ...
    elseif reactionDetail.hitChanceTerrainReactionUnit and reactionDetail.hitChanceTerrainReactionUnit[reactingUnitTerrain] then
        print("reaction unit terrain chance")
...
local function munitionEffect(triggerUnitBeforeDamage,reactingUnit,tableOfMunitionsGenerated,triggerUnitHit,damageToBeDoneToTriggerUnit)
    ...
    if reactionDetail.destroyMunitionsWithKill and (triggerUnitBeforeDamage.hitpoints-damageToBeDoneToTriggerUnit <=0)
    then
...
-- provide linkState from reactionBase
simpleReactions.linkState = reactionBase.linkState

globalDoReaction = doReaction
