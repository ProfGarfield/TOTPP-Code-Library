...
local function individualReaction(triggerUnit,reactingUnit,reactingUnitsTable,otherNearbyUnitsTable,
                    targetReactionDamage, 
                    isAreaDamageUnit, areaDamageToReactingUnit, areaDamageToNotReactingUnit,
                    postReactionFunction,killFunction)

    -- effect on other units
    if isAreaDamageUnit(triggerUnit,reactingUnit) then
        for index, otherReactingUnit in pairs(reactingUnitsTable) do
            if otherReactingUnit ~= reactingUnit then
                otherDamage = areaDamageToReactingUnit(otherReactingUnit,reactingUnit,triggerUnit)
                if otherDamage >= otherReactingUnit.hitpoints then
                    replaceUnit = killFunction(otherReactingUnit,reactingUnit)
                    if replaceUnit then
                        reactingUnitsTable[index] = replaceUnit
                    else
                        reactingUnitsTable[index] = nil
                    end
                else
                    otherReactingUnit.damage = otherReactingUnit.damage + otherDamage
                end
            end
        end --for index,otherReactingUnit
        for index, otherNearbyUnit in pairs(otherNearbyUnitsTable) do
            debugPrint("damage to this unit")
            debugPrint(otherNearbyUnit)
            local otherDamage = areaDamageToNotReactingUnit(otherNearbyUnit,reactingUnit,triggerUnit)
            debugPrint(otherDamage)
            if otherDamage >= otherNearbyUnit.hitpoints then
                replaceUnit = killFunction(otherNearbyUnit,reactingUnit)
                if replaceUnit then
                    otherNearbyUnitsTable[index] = replaceUnit
                else
                    otherNearbyUnitsTable[index] = nil
                end
            else
                otherNearbyUnit.damage = otherNearbyUnit.damage + otherDamage
            end
        end--for index,otherNearbyUnit
    end-- if isAreaDamageUnit
           ...
           reactingUnit = reactingUnitsTable[index]
            if willReact(currentTriggerUnit,reactingUnit,reactingUnitsTable,otherNearbyUnitsTable) then
                currentTriggerUnit = individualReaction(currentTriggerUnit,reactingUnit,reactingUnitsTable,
                                    otherNearbyUnitsTable,targetReactionDamage,isAreaDamageUnit,
                                    areaDamageToReactingUnit,areaDamageToNotReactingUnit,
                                    postReactionFunction,killFunction)
            end
