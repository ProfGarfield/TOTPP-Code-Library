...
local offerMercenaries = {[15]=true,[30]=true,[50]=true,}
local mercPrices = {[1] = 400,[2] = 1000,[3]=1500,[4]=2100}

local function doAfterProduction(turn,tribe)-->void
    --[[
    if tribe == object.tOrangeTribe and offerMercenaries[turn] then
        local mercMenu = {}
        mercMenu[1] = "Do not hire mercenaries."

        for numberOfMercs = 1,4 do
            if mercPrices[numberOfMercs] and mercPrices[numberOfMercs] <= tribe.money then
                -- we need to offset the menu, since menu slot 1 is used for
                -- not hiring any mercenaries
                mercMenu[numberOfMercs+1] = "Hire "..tostring(numberOfMercs).." "..object.uHessianGrenadiers.name.." unit for "..tostring(mercPrices[numberOfMercs]).." gold."
            end
        end
        local choice = text.menu(mercMenu,"Shall we hire some mercenaries?  Our treasury stands at "..tostring(tribe.money).." gold.","")
        -- if choice is 1, we don't do anything
        if choice >= 2 then
            local mercsHired = choice-1
            for i=1,mercsHired do
                local newMerc = civ.createUnit(object.uHessianGrenadiers,object.tOrangeTribe,civ.getTile(152,20,0))
                newMerc.homeCity = nil
            end
            tribe.money = tribe.money-mercPrices[mercsHired]
        end
    end-- end of mercenaryPurchase
    --]]
    if tribe == object.tOrangeTribe and offerMercenaries[turn] then
        local mercMenu = {}
        mercMenu[1] = "Do not hire mercenaries."
        if tribe.money >= 400 then
            mercMenu[2] = "Hire a "..object.uHessianGrenadiers.name.." unit and a "..object.uHessianMusketeers.name.." unit for 400 gold."
        end
        if tribe.money >= 1000 then
            mercMenu[3] = "Hire a "..object.uHessianGrenadiers.name.." unit, a "..object.uHessianMusketeers.name.." unit and a "..object.uHessianJaegerRegt.name.." unit for 1000 gold."
        end
        if tribe.money >= 1500 then
            mercMenu[4] = "Hire a "..object.uHessianMusketeers.name.." unit, a "..object.uvonKnyphausenRegt.name.." unit and a "..object.uHessianArtillery.name.." unit for 1500 gold."
        end
        if tribe.money >= 2100 then
            mercMenu[5] = "Hire Waldeck and von Rall's Grenadier regiments, and Hessian Jaegers, Dragoons, Artillery and Sappers for 2100 gold."
        end
        local choice = text.menu(mercMenu,"Shall we hire some mercenaries?  Our treasury stands at "..tostring(tribe.money).." gold.","")
        -- if the choice is 1, we don't have to do anything
        if choice == 2 then
            local newMerc = civ.createUnit(object.uHessianGrenadiers,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianMusketeers,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            tribe.money = tribe.money - 400
        ...
        elseif choice == 3 then
            ...
            newMerc = civ.createUnit(object.uHessianMusketeers,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianJaegerRegt,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            tribe.money = tribe.money - 1000
        elseif choice == 4 then
            local newMerc = civ.createUnit(object.uHessianMusketeers,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uvonKnyphausenRegt,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianArtillery,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            tribe.money = tribe.money - 1500
        elseif choice == 5 then
            local newMerc = civ.createUnit(object.uvonRallsGrenadierRegt,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uWaldeckRegt,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianJaegerRegt,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianDragoons,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianArtillery,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            newMerc = civ.createUnit(object.uHessianSappers,object.tOrangeTribe,civ.getTile(152,20,0))
            newMerc.homeCity = nil
            tribe.money = tribe.money - 2100
        end
    end
end
...
local plantationCity= {}
plantationCity[gen.getTileID(45,81,0)] = true -- Charles Town
plantationCity[gen.getTileID(48,76,0)] = true -- Wilmington

local function doOnCityTaken(city,defender) -->void
    legacy.doCityTakenEvents(city,defender)
    if city.owner == object.tOrangeTribe and defender == object.tCyanTribe and 
        plantationCity[gen.getTileID(city.location)] then
        local menuOptions = {}
        menuOptions[1] = "Free the slaves (receive 500 gold)"
        menuOptions[2] = "Conscript the slaves (receive one "..object.uEthiopianRegt.name..")."
        local menuMessage = "What shall we do with the slaves living on the plantations near "..city.name.."?"
        local choice = text.menu(menuOptions,menuMessage,"Local Slaves")
        if choice == 1 then
            city.owner.money = city.owner.money + 500
        elseif choice == 2 then
            local slaveSoldiers = civ.createUnit(object.uEthiopianRegt,object.tOrangeTribe,city.location)
            slaveSoldiers.homeCity = nil
        end
    end
end
...
local function doOnActivateUnit(unit,source) --> void
    attackBonus.simpleAttackBonus(unit,attackBonusSettings.simpleAttackBonusTable,attackBonusSettings.defaultAttackTable)
end
...
local function doOnKeyPress(keyCode)
    ...
    if keyCode == keyboard.tab and civ.getActiveUnit() then
        helpkey.helpKey(keyCode,keyboard.tab,{},{},helpkey.defaultUnitTextFunction)
    end
end
...
improvementAliases={}
improvementAliases.TradingCompany = civ.getImprovement(22)

unitAliases={}
unitAliases.Merchant = civ.getUnitType(49)

--[[
local function inBuildMenu(defaultBuildFunction,city,item)
if civ.isUnitType(item) then
if item == unitAliases.Merchant then
return civ.hasImprovement(city,improvementAliases.TradingCompany) and defaultBuildFunction(city,item)
end
end
